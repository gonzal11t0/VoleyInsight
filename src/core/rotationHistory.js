...
function limpiarCamposSincronizacion(punto) {
    const limpio = { ...punto };
    for (const campo of CAMPOS_SINCRONIZACION) delete limpio[campo];
    return limpio;
}

function cambioSincronizacion(original, actualizado) {
    return [...CAMPOS_SINCRONIZACION, 'sincronizacionOficial']
        .some(campo => original?.[campo] !== actualizado?.[campo]);
}
...
function enriquecerPuntosManuales(puntosManuales, snapshots) {
    ...
    const setsConHistorialOficial = new Set(
        rallies
            .filter(rally => rally.sincronizado)
            .map(rally => Number(rally.set) || 1)
    );
    ...
    const puntos = (Array.isArray(puntosManuales) ? puntosManuales : []).map(punto => {
        ...
        if (!rally) {
            pendientes++;
            const setPunto = Number(punto.set) || 1;
            if (!setsConHistorialOficial.has(setPunto)) {
                return { ...punto };
            }
            const pendiente = {
                ...limpiarCamposSincronizacion(punto),
                sincronizacionOficial: 'pendiente'
            };
            if (cambioSincronizacion(punto, pendiente)) cambios++;
            return pendiente;
        }
        if (punto.equipoAnota !== rally.equipoAnota) {
            ...
            if (cambioSincronizacion(punto, conflicto)) cambios++;
            return conflicto;
        }
        ...
        if (cambioSincronizacion(punto, enriquecido)) cambios++;
        return enriquecido;
    });
