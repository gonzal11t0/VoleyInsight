class VolleyballMetrics {
    ...
    calculateBreakPointEfficiency(){
        ...
        return { breaks: { home: homeBreaks, away: awayBreaks }, opportunities: { home: homeOpp, away: awayOpp }, efficiency: { home: homeEff, away: awayEff }, interpretation: interp };
    }
    ...
    calculateSideoutPercentage(){
        ...
        return { sideouts: { home: homeSide, away: awaySide }, totalServingPoints: { home: homeServes, away: awayServes }, percentage: { home: homePct, away: awayPct }, interpretation: interp };
    }
    ...
    calculateMomentumIndex(){
        ...
        for(let i = windowSize-1; i < this.snapshots.length; i++){
            ...
            momentumPoints.push({ point: i+1, momentum: parseFloat(((h-a)/windowSize).toFixed(2)), dominant: (h-a)/windowSize > 0.4 ? this.homeTeam : (h-a)/windowSize < -0.4 ? this.awayTeam : 'NEUTRAL' });
        }
        ...
        return { series: momentumPoints, peak: momentumPoints.find(m => Math.abs(m.momentum) === maxM), dominancePercentage: { home: (domH/total*100).toFixed(1), away: (domA/total*100).toFixed(1), neutral: (domN/total*100).toFixed(1) }, interpretation: interp };
    }
    ...
    calculateClutchPerformance(){
        ...
        return { totalClutchPoints: total, criticalClutchPoints: critical.length, efficiency: { home: homeEff, away: awayEff }, criticalEfficiency: { home: critical.length ? (homeCrit/critical.length*100).toFixed(1) : 0, away: critical.length ? (awayCrit/critical.length*100).toFixed(1) : 0 }, interpretation: interp };
    }

    calculatePhaseEfficiency(){
        const phases = { EARLY: { home:0, away:0, total:0 }, MID: { home:0, away:0, total:0 }, LATE: { home:0, away:0, total:0 } };
        this.snapshots.forEach(s => { if(s.scorer && phases[s.phase]){ phases[s.phase][s.scorer==='HOME'?'home':'away']++; phases[s.phase].total++; } });
        const eff = {
        ...
        const homeLate = parseFloat(eff.LATE.home), awayLate = parseFloat(eff.LATE.away), homeEarly = parseFloat(eff.EARLY.home), awayEarly = parseFloat(eff.EARLY.away);
    ...
    calculateRunImpact(){
        ...
        const responses = runs.map(run => {
            ...
            return { runTeam: run.team === 'HOME' ? this.homeTeam : this.awayTeam, runPoints: run.points, startPoint: run.startPoint, response: next ? (next === 'HOME' ? this.homeTeam : this.awayTeam) : null, wasEffective: next && next !== run.team };
        });
        ...
        return { runs: runs.map(r => ({ team: r.team === 'HOME' ? this.homeTeam : this.awayTeam, points: r.points, startPoint: r.startPoint, startScore: r.startScore })), responses, recoveryRate: rate, interpretation: interp };
    }
    ...
    generateFullReport(){
        ...
        return {
            matchInfo: { homeTeam: this.homeTeam, awayTeam: this.awayTeam, finalScore: last ? `${last.homeScore}-${last.awayScore}` : '0-0', totalPoints: total, winner: last ? (last.homeScore > last.awayScore ? this.homeTeam : this.awayTeam) : null },
            basicStats: { points: { home: homePts, away: awayPts }, pointsPercentage: { home: ((homePts/total)*100).toFixed(1), away: ((awayPts/total)*100).toFixed(1) } },
            advancedMetrics: { breakPointEfficiency: breaks, sideoutPercentage: sideout, momentumIndex: momentum, clutchPerformance: clutch, phaseAnalysis: phase, runImpact: runImpact },
            summary
        };
    }
    ...
    generateSummary(breaks, sideout, clutch, phase, runImpact){
        ...
        const hb = parseFloat(breaks.efficiency.home), ab = parseFloat(breaks.efficiency.away);
        if(hb > 45) insights.push(`⚡ ${this.homeTeam} es letal en breaks: ${hb}%`);
        if(ab > 45) insights.push(`⚡ ${this.awayTeam} es letal en breaks: ${ab}%`);
        const hc = parseFloat(clutch.efficiency.home), ac = parseFloat(clutch.efficiency.away);
    ...
    generateKeyTakeaways(breaks, sideout, clutch, phase){
        const takeaways = [];
        const hb = parseFloat(breaks.efficiency.home), hs = parseFloat(sideout.percentage.home);
        const ab = parseFloat(breaks.efficiency.away), aso = parseFloat(sideout.percentage.away);
