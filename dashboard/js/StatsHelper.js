...
export function renderizarTop5ConNombres(sl, sv, jugadoresLocal, jugadoresVisitante) {
    ...
    const todos = [];
    for (const [num, s] of Object.entries(sl)) { if (num === 'null' || num === 'NaN' || isNaN(parseInt(num))) continue; let ef = parseFloat(s.eficiencia); if (isNaN(ef)) ef = 0; if (ef > 100) ef = 100; todos.push({ num: parseInt(num), nombre: nl[num] || `Jugador ${num}`, equipo: 'LOCAL', puntos: s.puntos || 0, eficiencia: ef.toFixed(1), acesServicio: s.acesServicio || 0 }); }
    for (const [num, s] of Object.entries(sv)) { if (num === 'null' || num === 'NaN' || isNaN(parseInt(num))) continue; let ef = parseFloat(s.eficiencia); if (isNaN(ef)) ef = 0; if (ef > 100) ef = 100; todos.push({ num: parseInt(num), nombre: nv[num] || `Jugador ${num}`, equipo: 'VISITANTE', puntos: s.puntos || 0, eficiencia: ef.toFixed(1), acesServicio: s.acesServicio || 0 }); }
    ...
    container.innerHTML = top5.map((j, idx) => `<div class="flex justify-between items-center p-2 bg-dark/30 rounded-lg hover:bg-dark/50 transition-all"><div class="flex items-center gap-2"><span class="text-xl">${medallas[idx]}</span><div><span class="font-semibold">${j.nombre}</span><span class="text-xs ml-2 px-2 py-0.5 rounded-full ${j.equipo === 'LOCAL' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}">${j.equipo}</span></div></div><div class="flex gap-4"><span class="text-primary font-bold">${j.puntos} pts</span><span class="text-gray-400 text-sm">Efi: ${j.eficiencia}%</span>${j.acesServicio > 0 ? `<span class="text-blue-400 text-sm">🎯 ${j.acesServicio} aces</span>` : ''}</div></div>`).join('');
}
...
export function renderizarGraficoPuntos(stats, equipo, jugadoresLocal, jugadoresVisitante, chartPuntosJugadores) {
    ...
    return new Chart(ctx, { type: 'bar', data: { labels: jug.map(j => j.nombre.length > 20 ? j.nombre.substring(0,18)+'...' : j.nombre), datasets: [{ label: 'Puntos', data: jug.map(j => j.puntos), backgroundColor: equipo === 'LOCAL' ? '#3b82f6' : '#ef4444', borderRadius: 4 }] }, options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color: '#fff' } }, tooltip: { callbacks: { label: ctx => `${ctx.raw} puntos` } } }, scales: { y: { ticks: { color: '#9ca3af' }, grid: { color: '#1f2937' } }, x: { ticks: { color: '#9ca3af', maxRotation: 45, minRotation: 45 } } } } });
}
