...
export class AnotadorManager {
   ...
   async init() {
    ...
    mostrarFeedback(msg, tipo = 'success') {
        ...
        if (tipo === 'success') fb.className = 'fixed bottom-20 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-sm font-bold bg-green-500 text-black z-50';
        else if (tipo === 'error') fb.className = 'fixed bottom-20 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-sm font-bold bg-red-500 text-white z-50';
        else if (tipo === 'info') fb.className = 'fixed bottom-20 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full text-sm font-bold bg-blue-500 text-white z-50';
        setTimeout(() => fb.classList.add('hidden'), 2000);
    }
    
    async guardarTimeout() {
        if (!this.estado.equipo) { this.mostrarFeedback('❌ Seleccioná un equipo primero', 'error'); return; }
        const timeout = { 
    ...
    actualizarCancha() {
        ...
        if (this.estado.equipo === 'LOCAL') {
            jugadoresLista = this.estado.local.jugadoresLista;
        } else {
            jugadoresLista = this.estado.visitante.jugadoresLista;
        }
    ...
    if (this.config.local.jugadores.length > 0 || this.config.visitante.jugadores.length > 0) {
        return true;
    }
  ...
  determinarEquipoQueAnota() {
    // ERROR: punto para el otro equipo (el rival)
    if (this.estado.accion === 'ERROR') {
        return this.estado.equipo === 'LOCAL' ? 'VISITANTE' : 'LOCAL';
    }
    // FALTA: también es punto para el otro equipo
    if (this.estado.accion === 'FALTA') {
        return this.estado.equipo === 'LOCAL' ? 'VISITANTE' : 'LOCAL';
    }
    // Las demás acciones: punto para el equipo que seleccionaste
    ...
    const punto = {
        ...
        jugador: jugadorQueAnota,  // Ya no hay condición especial
    ...
    deshacer() {
        if (!this.estado.historial.length) { this.mostrarFeedback('⚠️ No hay puntos para deshacer', 'error'); return; }
    ...
    navegarJugador(direccion) {
        ...
        for (let i = 0; i < btns.length; i++) {
            if (btns[i].classList.contains('seleccionado')) { currentIndex = i; break; }
        }
