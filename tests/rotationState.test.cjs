...
const pendienteConRotacionVieja = enriquecerPuntosManuales([
    {
        set: 1,
        equipoAnota: 'LOCAL',
        marcadorDespues: '99-99',
        rotacionLocal: 5,
        rotacionVisitante: 4,
        rotacionLocalDespues: 6,
        rotacionVisitanteDespues: 4,
        eventoOficial: 'SIDEOUT_HOME',
        sincronizacionOficial: 'confirmada'
    }
], snapshots);
assert.equal(pendienteConRotacionVieja.puntos[0].sincronizacionOficial, 'pendiente');
assert.equal(pendienteConRotacionVieja.puntos[0].rotacionLocal, undefined);
assert.equal(pendienteConRotacionVieja.puntos[0].eventoOficial, undefined);
assert.equal(pendienteConRotacionVieja.cambios, 1, 'debe guardar la limpieza de una sincronización obsoleta');

const sinHistorialOficial = enriquecerPuntosManuales([
    {
        set: 1,
        equipoAnota: 'LOCAL',
        marcadorDespues: '16-11',
        rotacionLocal: 5,
        rotacionVisitante: 5,
        rotacionLocalDespues: 6,
        rotacionVisitanteDespues: 5,
        eventoOficial: 'SIDEOUT_HOME',
        sincronizacionOficial: 'confirmada'
    }
], [{ set: 1, homeScore: 15, awayScore: 12, scorer: null }]);
assert.equal(sinHistorialOficial.puntos[0].rotacionLocal, 5);
assert.equal(sinHistorialOficial.puntos[0].eventoOficial, 'SIDEOUT_HOME');
assert.equal(sinHistorialOficial.puntos[0].sincronizacionOficial, 'confirmada');
assert.equal(sinHistorialOficial.cambios, 0, 'un estado final aislado no debe borrar el respaldo histórico');

const conflictoConRotacionVieja = enriquecerPuntosManuales([
    {
        set: 1,
        equipoAnota: 'LOCAL',
        marcadorDespues: '0-1',
        rotacionLocal: 1,
        rotacionVisitante: 1,
        eventoOficial: 'BREAK_HOME',
        sincronizacionOficial: 'confirmada'
    }
], snapshots);
assert.equal(conflictoConRotacionVieja.puntos[0].sincronizacionOficial, 'equipo_no_coincide');
assert.equal(conflictoConRotacionVieja.puntos[0].equipoAnotaOficial, 'VISITANTE');
assert.equal(conflictoConRotacionVieja.puntos[0].rotacionLocal, undefined);
assert.equal(conflictoConRotacionVieja.puntos[0].eventoOficial, undefined);
